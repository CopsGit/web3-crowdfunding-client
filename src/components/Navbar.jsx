...
const Navbar = () => {
    ...
    const shorterAddress = address ? `${address.slice(0, 4)}...${address.slice(-4)}` : 'Connect Wallet';
        ...
        <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
            ...
            <div className="sm:flex hidden flex-row justify-end gap-4">
                <CustomButton
                    ...
                    styles={address ? 'bg-[#1dc071] hover:bg-[#4acd8d]' : 'bg-[#8c6dfd]'}
                    handleClick={() => {
                ...
                <Link to="/profile" className={`min-h-[52px] px-4 flex justify-between `}>
                    <div
                        className="w-[52px] h-[52px] rounded-full bg-[#2c2f32] flex justify-center items-center cursor-pointer">
                        <img src={thirdweb} alt="user" className="w-[60%] h-[60%] object-contain"/>
                    </div>
                    <div className={
                        `flex flex-col justify-center p-3 items-center gap-1 cursor-pointer ${isActive === 'profile' ? 'text-[#1dc071]' : 'text-[#4b5264]'}`
                    }>
                        <p className="text-[#fff] text-[16px] font-epilogue font-semibold leading-[18px]">{shorterAddress}</p>
                    </div>
                </Link>
